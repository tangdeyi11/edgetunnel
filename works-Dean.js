...
let sub = '';// 留空则使用内置订阅
let subconverter = 'SUBAPI.fxxk.dedyn.io';// clash订阅转换后端，目前使用CM的订阅转换功能subapi-loadbalancing.pages.dev。也可以使用肥羊的订阅转换后端url.v1.mk，自带虚假uuid和host订阅。
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader, log,) {
	...
	async function connectAndWrite(address, port, socks = false) {
		...
		if (address.includes('fast.com') || address.includes('netflix.com') || address.includes('netflix.net') || address.includes('nflxext.com') || address.includes('nflxso.net') || address.includes('nflxvideo.net') || address.includes('nflxsearch.net') || address.includes('nflximg.com') || address.includes('nflximg.com')) {
...
async function handleDNSQuery(udpChunk, webSocket, vlessResponseHeader, log) {
	...
	//上面的udpChunk参数的意思是仅处理udp包，本段代码就是udp的DNS包，如果DNS查询采用TCP，本段代码不生效。如果要同时支持UDP和TCP的DNS请求，需要求改udpChunk为dnsQueryData
    try {
